...
// ---- Seeding (ladder -> summonerIds) ----
async function seedSummonerIdsForPlatform(platform: Platform): Promise<string[]> {
  const ids: string[] = [];

  // Masters+ (full list, no paging)
...
// ---- Fetch matches ----
async function pullMatchesForPlatform(
  platform: Platform,
  puuids: string[],
  per: number
): Promise<any[]> {
  ...
  const matches = (
    await Promise.all(
      [...idSet].map((id) =>
        limitMatch(async () => {
          try {
            return await getMatch(region, id, RIOT_API_KEY);
          } catch (e: any) {
            console.warn(`[Match] ${platform}/${region} id=${id} failed: ${e?.message || e}`);
            return null;
          }
        })
      )
  ...
  console.log(`[Matches] ${platform} fetched=${(matches as any[]).length}`);
  return matches as any[];
}

// ---- State load/save (single combined file per patch) ----
const outDir = "data";
...
type RawState = {
  ...
  __comp: Array<{
    ...
    sumPlacement: number;
    unit_set: string[];
    units: Array<[string, Array<[string, number]>]>; // [cid, [[item,count],...]]
  }>;
  __unitCombos: Array<{
    unit: string;
    combos: Array<[string, { picks: number; wins: number; sumPlacement: number }]>;
  }>;
};
...
function deserializeCompBuckets(rows: RawState["__comp"]) {
  ...
  for (const r of rows) {
    ...
    m.set(r.key, {
      patch: r.patch,
      comp_key: r.key.split("::")[1],
      picks: r.picks,
      wins: r.wins,
      sumPlacement: r.sumPlacement,
      units,
      unit_set: r.unit_set,
    });
  }
...
function serializeUnitCombos(
  map: Map<string, Map<string, { picks: number; wins: number; sumPlacement: number }>>
) {
...
function deserializeUnitCombos(
  rows: RawState["__unitCombos"]
): Map<string, Map<string, { picks: number; wins: number; sumPlacement: number }>> {
...
(async () => {
  console.log(
    `[Config] patch=${PATCH} platforms=${PLATFORMS} seed=${SEED_SUMMONERS} per=${MATCHES_PER}`
  );

  // Resume state if same patch; reset if patch changed
  const prev = await loadPrevRaw();
  let compBuckets = new Map<string, CompCounts>();
  let unitComboBuckets = new Map<
    string,
  ...
  for (const platform of platforms) {
    ...
    if (ids.length === 0) {
      console.error(`[Fatal] ${platform}: no summonerIds from league endpoints.`);
      continue;
    }
    ...
    if (puuids.length === 0) {
      console.error(`[Fatal] ${platform}: converted 0 PUUIDs.`);
      continue;
    }
  ...
  const unit_item_meta = finalizeUnitCombos(
    unitComboBuckets,
    Number(MIN_PICKS_ITEM_COMBO),
    10
  );
  ...
  console.log(`[Done] wrote ${outPathVersioned} and ${outPathCurrent}`);
})().catch((e) => {
  console.error(e);
  process.exit(1);
});
